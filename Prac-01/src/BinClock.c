...
void hourInc(void){
	...
	if (interruptTime - lastInterruptTime>200){
		...
		if (tempHour > 0x22) {
			...
			tempHour = tempHour + 0x01;
		}
		//Write hours back to the RTC
		wiringPiI2CWriteReg8(RTC, HOUR_REGISTER, tempHour);
	}
...
void minInc(void){
	...
	if (interruptTime - lastInterruptTime>200){
		...
		//Fetch RTC Time
		tempMin = wiringPiI2CReadReg8(RTC, MIN_REGISTER);
		//Increase minutes by 1, ensuring not to overflow
		 if (tempMin >  0x58) {
			tempMin = 0x00;
		} else {
			tempMin = tempMin + 0x01;
		}
		//Write minutes back to the RT
		wiringPiI2CWriteReg8(RTC, MIN_REGISTER, tempMin);
	}
	lastInterruptTime = interruptTime;
}
